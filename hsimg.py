"""Research-grade prototype for 3D Hierarchical Semantic Indoor Mobility Graphs.

The module intentionally separates IFC evidence, geometric inference and graph
assembly.  Every inferred relation carries ``relation_source`` and ``confidence``.
It targets IFC2X3/IFC4 models and degrades to partial, explicitly diagnosed output
when geometry or semantic relations are incomplete.
"""
...
class HSIMGBuilder:
    ...
    def build_horizontal_subgraphs(self) -> list[dict[str, Any]]:
        ...
        for space in self.spaces.values():
            ...
            extraction_method = "raster_medial_axis"
            if not axes:
                axes = _fallback_visibility_axis(space.footprint, access_points, space.elevation, self.config.spatial_tolerance_m)
                extraction_method = "fallback_visibility_graph"
                self._issue("warning", "skeleton_fallback", f"Used visibility fallback for {space.name}", "Install scikit-image or adjust skeleton resolution", related_ifc_guid=space.ifc_guid, related_node_id=space.space_id, geometry=space.interior_point)
            ...
            for axis_index, axis in enumerate(axes):
                ...
                self.mobility_axes.append({
                    ...
                    "metadata_json": _json({"resolution_m": self.config.skeleton_resolution_m}), "geometry": axis,
                })
            ...
            for axis_index, axis in enumerate(axes):
                ...
                for piece in pieces:
                    ...
                    self._add_bidirectional_edge(endpoint_ids[0], endpoint_ids[1], edge_type="internal_axis",
                        mobility_mode="walk", subgraph_id=subgraph_id, geometry=piece,
                        relation_source="geometry_inferred", confidence=0.90)
            self.subgraphs.append({
                ...
                "parameters_json": _json({"resolution_m": self.config.skeleton_resolution_m, "pruning_m": self.config.medial_axis_pruning_length_m}),
